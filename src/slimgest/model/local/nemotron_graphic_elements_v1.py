...
class NemotronGraphicElementsV1(BaseModel):
    ...
    def invoke(self, input_data: torch.Tensor, orig_shape: Tuple[int, int]) -> Union[List[Dict[str, torch.Tensor]], Dict[str, torch.Tensor]]:
    ...
    def invoke_remote(
        ...
        *,
        ...
        """
        Invoke the graphic elements NIM endpoint over HTTP.
        ...
        if isinstance(input_data, list):
            ...
            raise ValueError(f"Unsupported input type/shape for remote graphic elements: {type(input_data)}")
        ...
        for i in range(0, len(imgs), bs):
            ...
            if per_list is None:
                if len(chunk) != 1:
                    raise RuntimeError("Remote graphic elements response did not contain a per-image list.")
                per_list = [data]
            if len(per_list) != len(chunk):
                raise RuntimeError(
                    f"Remote graphic elements response size mismatch: got {len(per_list)} outputs for {len(chunk)} inputs."
                )
