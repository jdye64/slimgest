...
class NemotronPageElementsV3NIM(NvidiaNIMModel):
    ...
    def invoke(self, input_data: Union[Path, str, BytesIO]) -> Any:
        ...
        payload = {
            "input": [
                {
                "type": "image_url",
                "url": f"data:image/png;base64,{input_data}"
                }
            ]
        }
    ...
    def postprocess(self, preds: Dict[str, Any]) -> Tuple[Union[List[str], torch.Tensor], Union[List[str], torch.Tensor],Union[List[str], torch.Tensor]]:
